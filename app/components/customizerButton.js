import Link from "next/link";

const CUSTOMIZER_VARIANTS = {
    Chapter: {
        href: "/painter",
        typeColour: "btn-primary",
        labelNoun: "chapter",
    },
    Chaos: {
        href: "/chaos-painter",
        typeColour: "btn-accent",
        labelNoun: "warband",
    },
    Eldar: {
        href: "/warhost-painter",
        typeColour: "btn-info",
        labelNoun: "warhost",
    },
};

export default function CustomizerButton({
    variant = "Chapter",
    label,
}) {
    const config = CUSTOMIZER_VARIANTS[variant] ?? CUSTOMIZER_VARIANTS.Chapter;
    const { href, typeColour, labelNoun } = config;
    const aria = label ?? `Customize your ${labelNoun}`;

    return (
        <Link
            className={`btn ${typeColour} btn-block rounded-sm`}
            href={href}
            aria-label={aria}
            title={aria}
        >
            <svg
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 384 512"
                className="size-5 fill-current"
                aria-hidden="true"
            >
                <path d="M162.4 6c-1.5-3.6-5-6-8.9-6l-19 0c-3.9 0-7.5 2.4-8.9 6L104.9 57.7c-3.2 8-14.6 8-17.8 0L66.4 6c-1.5-3.6-5-6-8.9-6L48 0C21.5 0 0 21.5 0 48L0 224l0 22.4L0 256l9.6 0 364.8 0 9.6 0 0-9.6 0-22.4 0-176c0-26.5-21.5-48-48-48L230.5 0c-3.9 0-7.5 2.4-8.9 6L200.9 57.7c-3.2 8-14.6 8-17.8 0L162.4 6zM0 288l0 32c0 35.3 28.7 64 64 64l64 0 0 64c0 35.3 28.7 64 64 64s64-28.7 64-64l0-64 64 0c35.3 0 64-28.7 64-64l0-32L0 288zM192 432a16 16 0 1 1 0 32 16 16 0 1 1 0-32z" />
            </svg>
            <span>{label ?? "Customize"}</span>
        </Link>
    );
}
