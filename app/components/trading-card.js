...
export default function TradingCard({
    ...
    return (
        <div id={id} className="card w-full max-w-96 bg-white text-neutral border-8 border-yellow-600">
            <div className="card-body p-2 m-0 rounded-lg">
                ...
                {/* Swatches */}
                <div className="join join-horizontal w-full" aria-label="Color palette">
                    {swatchIndices.map((i) => {
                        const name = colors[i]?.name ?? "";
                        const hex = colors[i]?.hex ?? "#000";
                        return (
                            <div
                                key={i}
                                className="join-item h-8 w-full border border-base-300"
                                data-tip={name}
